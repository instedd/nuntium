...
Nuntium::Application.routes.draw do
  ...
  scope '/:account_id/clickatell', :constraints => {:account_id => /.*/} do
    get '/incoming' => 'clickatell#index', :as => :clickatell
    get '/ack' => 'clickatell#ack', :as => :clickatell_ack, :constraints => {:account_id => /.*/}
  end

  get '/clickatell/view_credit' => 'clickatell#view_credit', :as => :clickatel_credit

  scope '/:account_id/:channel_id/nexmo/:callback_token', :constraints => {:account_id => /.*/, :channel_id => /.*/} do
    get '/incoming' => 'nexmo#incoming', :as => :nexmo_incoming, :constraints => {:account_id => /.*/, :channel_id => /.*/}
    get '/ack' => 'nexmo#ack', :as => :nexmo_ack, :constraints => {:account_id => /.*/, :channel_id => /.*/}
  end
  ...
  #   get '/incoming/:callback_guid' => 'shujaa#index', :as => :shujaa
  # end

  scope '/twitter' do
    get '/callback' => 'twitter#callback', :as => :twitter_callback
  end
  ...
  scope '/:account_id/qst', :constraints => {:account_id => /.*/}  do
    ...
    get '/setaddress' => 'qst_server#set_address', :as => :qst_set_address, :constraints => {:account_id => /.*/}
  end
  ...
  scope '/:account_name/:application_name', :constraints => {:account_name => /.*/, :application_name => /.*/} do
    ...
    get '/send_ao' => 'ao_messages#create_via_api', :as => :send_ao, :constraints => {:account_name => /.*/, :application_name => /.*/}
    get '/get_ao' => 'ao_messages#get_ao', :as => :get_ao, :constraints => {:account_name => /.*/, :application_name => /.*/}
  end

  get '/:account_id/dtac/incoming' => 'dtac#index', :as => :dtac, :constraints => {:account_id => /.*/}
  # TODO: are these necessary/correct? taken from production environment
  get '/dtac/geochat/incoming' => 'dtac#index', :account_id => 'instedd'
  get '/dtac/:account_id/incoming' => 'dtac#index'
  ...
  scope '/:account_id/twilio', :constraints => {:account_id => /.*/} do
    get '/incoming' => 'twilio#index', :as => :twilio
    get '/ack' => 'twilio#ack', :as => :twilio_ack
  end

  scope '/api' do
    scope '/carriers' do
      get '/' => 'api_carrier#index', :as => :carriers
      get '/:guid' => 'api_carrier#show', :as => :carrier
    end

    scope '/countries' do
      get '/' => 'api_country#index', :as => :countries
      get '/:iso' => 'api_country#show', :as => :country
    end
    ...
    scope '/custom_attributes' do
      ...
      post '/' => 'api_custom_attributes#create_or_update', :as => :api_custom_attributes_show_post
    end
