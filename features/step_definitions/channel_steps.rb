...
When /^(?:a|an) "([^\"]*)" channel named "([^\"]*)" with "([^\"]*)" set to "([^\"]*)" belongs to the "([^\"]*)" account$/ do |chan_kind, chan_name, field_name, field_value, account_name|
  a = Account.find_by_name account_name
  raise "Account named \"#{account_name}\" does not exist" unless a
...
When /^a "([^\"]*)" channel named "([^\"]*)" with an at rule that sets "([^\"]*)" to "([^\"]*)" when "([^\"]*)" "([^\"]*)" "([^\"]*)" belongs to the "([^\"]*)" account$/ do |chan_kind, chan_name, at_rule_set_field, at_rule_set_value, at_rule_match_key, at_rule_match_operator, at_rule_match_value, account_name|
  a = Account.find_by_name account_name
  raise "Account named \"#{account_name}\" does not exist" unless a
