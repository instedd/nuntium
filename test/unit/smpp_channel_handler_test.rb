...
class SmppChannelHandlerTest < ActiveSupport::TestCase
  test "should not save if host is blank" do
    ...
    chan.configuration = {:port => 3200, :user => 'user', :password => 'password' }
    assert !chan.save
  end
  ...
  test "should not save if user is blank" do
    app = Application.create(:name => 'app', :password => 'foo')
    chan = Channel.new(:application_id => app.id, :name => 'chan', :kind => 'smpp', :protocol => 'smpp')
    chan.configuration = {:host => 'host', :port => 3200, :password => 'password' }
    assert !chan.save
  end
  ...
  test "should not save if password is blank" do
    app = Application.create(:name => 'app', :password => 'foo')
    chan = Channel.new(:application_id => app.id, :name => 'chan', :kind => 'smpp', :protocol => 'smpp')
    chan.configuration = {:host => 'host', :port => 3200, :user => 'user' }
    assert !chan.save
  end
  ...
  test "should not save if port is negative" do
    app = Application.create(:name => 'app', :password => 'foo')
    chan = Channel.new(:application_id => app.id, :name => 'chan', :kind => 'smpp', :protocol => 'smpp')
    chan.configuration = {:host => 'host', :port => -3200, :user => 'user', :password => 'password' }
    assert !chan.save
  end
