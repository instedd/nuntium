...
class MessageControllerTest < ActionController::TestCase
  ...
  test "mark ao messages as cancelled" do
    ...
    assert_redirected_to(:controller => 'home', :action => 'ao_messages', :ao_messages => [@ao_msg1.id, @ao_msg2.id])
  ...
  test "mark ao messages as cancelled using search" do
    get :mark_ao_messages_as_cancelled, {:ao_all => 1, :search => 'one'}, {:account_id => @account.id}

    assert_redirected_to(:controller => 'home', :action => 'ao_messages', :ao_all => 1, :search => 'one')
    assert_equal '2 Application Originated messages were marked as cancelled', flash[:notice]
  ...
  test "mark at messages as cancelled" do
    ...
    assert_redirected_to(:controller => 'home', :action => 'at_messages', :at_messages => [@at_msg1.id, @at_msg2.id])
  ...
  test "mark at messages as cancelled using search" do
    get :mark_at_messages_as_cancelled, {:at_all => 1, :search => 'one'}, {:account_id => @account.id}

    assert_redirected_to(:controller => 'home', :action => 'at_messages', :at_all => 1, :search => 'one')
    assert_equal '2 Application Terminated messages were marked as cancelled', flash[:notice]
  ...
  test "re-route ao messages using search" do
    get :reroute_ao_messages, {:ao_all => 1, :search => 'one'}, {:account_id => @account.id}
