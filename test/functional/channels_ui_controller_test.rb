...
class ChannelsUiControllerTest < ActionController::TestCase
  def setup
    ...
    @qst_server_params = { "kind" => "qst_server", "config" => { "name" => "foo", "ticket_code" => @ticket.code } }
  end
  ...
  test "creates QST Server channel" do
    post :create, @qst_server_params
    ...
    assert_equal @account.id, chan.account_id
    assert_equal chan.direction, Channel::Bidirectional
    assert_equal chan.protocol, QstServerChannel.default_protocol

    assert_equal chan.name, @qst_server_params['config']['name']
    assert_equal chan.address, @ticket[:data][:address]
    t = Ticket.find @ticket.id
    assert chan.authenticate(t[:data][:password])
  end

  test "updates Twilio channel" do
    post :create, @twilio_params
    chan_before = Channel.all[0]
    twilio_config_v2 = { "account_sid" => "545454_v2", "auth_token" => "656565_v2", "from" => "123456782" }

    post :update, :config => twilio_config_v2, :id => chan_before.name
    chan = Channel.all[0]
    ...
    assert_equal chan.name, chan_before.name
  ...
  test "updates Chikka channel" do
    ...
    post :update, :config => chikka_config_v2, :id => chan_before.name
    chan = Channel.all[0]
  ...
  test "updates AfricasTalking channel" do
    ...
    post :update, :config => africas_talking_config_v2, :id => chan_before.name
    chan = Channel.all[0]
  ...
  test "updates SMPP channel" do
    ...
    post :update, :config => smpp_config_v2, :id => chan_before.name
    chan = Channel.all[0]
  ...
  test "updates QST Server channel" do
    post :create, @qst_server_params
    chan_before = Channel.all[0]
    ticket_2 = Ticket.make :pending, :data => { :address => '8347-2345' }
    qst_server_params_v2 = { "ticket_code" => ticket_2.code }

    post :update, :config => qst_server_params_v2, :id => chan_before.name
    chan = Channel.all[0]
    ...
    assert_equal chan.name, chan_before.name
    assert_equal chan.address, ticket_2[:data][:address]
    ticket_2 = Ticket.find ticket_2.id
    assert chan.authenticate(ticket_2[:data][:password])
  end
